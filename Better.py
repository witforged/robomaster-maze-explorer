...
class MazeSolver:
    ...
    TARGET_DISTANCE_M = 0.04    # อยากห่างกำแพง ~5 ซม.
    TOLERANCE_M = 0.02           # ยอมผิดได้ ±2 ซม.
    MAX_LATERAL_STEP_M = 0.05    # จำกัดระยะสไลด์ต่อครั้ง
    MAX_FORWARD_STEP_M = 0.05    # จำกัดระยะเดินหน้า/ถอยต่อครั้ง
    ...
    def scan_and_align(self):
        """
        สแกน L/F/R ครั้งเดียวด้วยกิมบอล จากนั้นคำนวณการสไลด์/เดินหน้า-ถอยหลัง
        เพื่อให้หุ่นอยู่กลางทางและ ~10cm จากกำแพง ก่อนคืนระยะเดิมไปใช้ทำแผนที่
        """
        d = self.ctrl.eye()  # cm
        l_cm, f_cm, r_cm = d.get("L"), d.get("F"), d.get("R")
        print(f"Scan distances (cm): L={l_cm}, F={f_cm}, R={r_cm}")

        # แปลงเป็นเมตร ถ้าค่าไหน None แปลว่าวัดไม่ได้/ไกลเกิน
        ...
        # ----- จัดให้อยู่ "กลาง" ทางเดินเมื่อเห็นซ้ายและขวา -----
        if l is not None and r is not None:
            # ถ้า l > r แปลว่าช่องซ้ายกว้างกว่า → สไลด์ไปทางซ้ายครึ่งของส่วนต่าง
            lateral_err = l - r
            ...
            # ----- ถ้าเห็นฝั่งเดียว ให้รักษา 10cm จากฝั่งนั้น -----
            if l is not None:
        ...
        # ----- สั่งขยับ (ใช้ชุดระยะเดียว ไม่สแกนซ้ำ) -----
        ...
        return d  # คืนค่าเป็นเซนติเมตร ใช้ต่อใน mapping ได้เลย
    ...
    def explore(self):
        ...
        while self.path_stack:
            ...
            # >>> NEW: สแกน+จัดตำแหน่งครั้งเดียว แล้วใช้ระยะเดียวกันสร้างแผนที่
            ...
            if current_cell not in self.maze_map:
                self._scan_and_map(current_cell, distances)  # >>> NEW: ส่งระยะที่เพิ่งสแกนไปใช้
    ...
    def _scan_and_map(self, cell, distances=None):
        if distances is None:
            ...
            print(f"Cell {cell} mapping with reused scan (no extra spin).")
