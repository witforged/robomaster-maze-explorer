# -*- coding: utf-8 -*-
"""
Merged RoboMaster DFS Maze Explorer
— รวมฟีเจอร์จากโค้ดสองชุด โดยคงพฤติกรรมหลัก (movement/decision) แบบโค้ดแรก

Highlights
- คงลอจิกการเคลื่อนที่/สแกน/จัดตำแหน่งของโค้ดแบบแรก
- เพิ่มระบบทำแผนที่กำแพงแบบมีสไตล์ (solid/dashed) เพื่อแสดงโซนนอกแผนที่ (วาดเท่านั้น)
- มีตัวเลือกเปิด/ปิดการ backtrack อัตโนมัติเมื่อออกนอกแผนที่ (ปิดไว้เป็นค่าเริ่มต้น เพื่อคงพฤติกรรมเดิม)
- เก็บ vision markers แบบเดิม (detect-only)

ปรับได้ในคลาส MazeSolver:
- OUTSIDE_DETECT_ENABLED: เปิดการตรวจ "นอกแผนที่" เพื่องานวาด
- OUTSIDE_AUTO_BACKTRACK: เปิดเพื่อบังคับ backtrack ถ้าอยู่นอกแผนที่ (ค่าเริ่มต้น False)
"""
...
def plot_maze(current_cell, visited, walls, path_stack, title="Real-time Maze Exploration"):
    ...
    # วาดกำแพง: รองรับทั้ง set (legacy) และ dict (style)
...
class MazeSolver:
    ...
    CELL_SIZE = 0.6              # m
    MAX_MAZE_WIDTH_M = 7 * 0.6   # สำหรับตรวจโซนนอกแผนที่แบบคร่าว ๆ (ใช้เพื่อแสดงผล)
    ...
    # ตัวเลือกการใช้งานฟีเจอร์ "นอกแผนที่"
    OUTSIDE_DETECT_ENABLED = True       # ใช้เพื่อวาดกำแพงเส้นประเท่านั้น (logic เดินเหมือนเดิม)
    OUTSIDE_AUTO_BACKTRACK = True      # ถ้า True จะ backtrack อัตโนมัติเมื่ออยู่นอกแผนที่
    ...
    # >>> สแกน L/F/R ครั้งเดียว + จัดตำแหน่ง ตามโค้ดแรก (คงพฤติกรรม)
    #     เพิ่มสถานะ is_outside เพื่อการวาด (ไม่บังคับเปลี่ยนการเดิน)
    def scan_and_align(self):
        d = self.ctrl.eye()  # cm
        l_cm, f_cm, r_cm = d.get("L"), d.get("F"), d.get("R")
        print(f"Scan distances (cm): L={l_cm}, F={f_cm}, R={r_cm}")

        # --- คำนวณโซนนอกแผนที่ (สำหรับการวาด/ทางเลือก) ---
        is_outside = False
        if self.OUTSIDE_DETECT_ENABLED and (l_cm is not None and r_cm is not None):
            current_width_m = (l_cm + r_cm) / 100.0
            if current_width_m >= self.MAX_MAZE_WIDTH_M:
                is_outside = True

        # --- Alignment เหมือนโค้ดแรก ---
        ...
        # ให้อยู่กลางทางเมื่อเห็นซ้าย/ขวา
        if l is not None and r is not None:
            ...
            # เกาะระยะฝั่งเดียวถ้าเห็นเพียงด้านเดียว
            if l is not None:
                e = self.TARGET_DISTANCE_M - l
                if abs(e) > self.TOLERANCE_M:
                    if e > 0:
                        moves["slide_right"] = float(np.clip(e, 0.0, self.MAX_LATERAL_STEP_M))
                    else:
                        moves["slide_left"]  = float(np.clip(-e, 0.0, self.MAX_LATERAL_STEP_M))
            if r is not None:
                e = self.TARGET_DISTANCE_M - r
                if abs(e) > self.TOLERANCE_M:
                    if e > 0:
                        moves["slide_left"]  = max(moves["slide_left"],  float(np.clip(e, 0.0, self.MAX_LATERAL_STEP_M)))
                    else:
                        moves["slide_right"] = max(moves["slide_right"], float(np.clip(-e, 0.0, self.MAX_LATERAL_STEP_M)))

        # ด้านหน้าให้ ~TARGET_DISTANCE_M เช่นกัน
        if f is not None:
            e = self.TARGET_DISTANCE_M - f
            if abs(e) > self.TOLERANCE_M:
                if e > 0:
                    moves["backward"] = float(np.clip(e, 0.0, self.MAX_FORWARD_STEP_M))
                else:
                    moves["forward"]  = float(np.clip(-e, 0.0, self.MAX_FORWARD_STEP_M))

        # สั่งขยับตามชุดระยะนี้ (ไม่สแกนซ้ำ)
    ...
    def explore(self):
        ...
        while self.path_stack:
            ...
            # ถ้าเปิดโหมด backtrack อัตโนมัติเมื่ออยู่นอกแผนที่
            if self.OUTSIDE_AUTO_BACKTRACK and is_outside:
    ...
    def _scan_and_map(self, cell, distances=None, is_outside=False):
        ...
        # --- HARD FENCE: ถ้าอยู่นอกแผนที่ ให้ปิดทุกทิศทาง (ไม่บันทึกเป็นทางเปิด)
        #     และวาดกำแพงแบบเส้นประสำหรับทิศที่เปิด (เพื่อแสดงว่าเป็นนอกแผนที่)
        if self.OUTSIDE_DETECT_ENABLED and is_outside:
            ...
            for move_key in ["L", "F", "R"]:
                ...
                if dist_cm is not None and dist_cm > self.WALL_THRESHOLD:
                    self.walls[wall_tuple] = 'dashed'   # เปิดสู่ภายนอก → วาดเส้นประ
                else:
                    self.walls[wall_tuple] = 'solid'    # ปิดจริง → เส้นทึบ

            self.maze_map[cell] = open_directions  # ว่าง → บังคับให้ backtrack ธรรมชาติ
            print(f"Mapped {cell} with open directions: [] (HARD FENCE)")
            print(f"Walls updated: {len(self.walls)} total walls.")
            return

        # --- ปกติ: ใช้กฎเดิม (เหมือนเวอร์ชันแรก) ---
        for move_key in ["L", "F", "R"]:
            direction = relative_dirs[move_key]
            dist_cm = distances.get(move_key)
            neighbor = self._get_target_coordinates(cell[0], cell[1], direction)
