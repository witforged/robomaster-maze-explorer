...
class Control:
    ...
    def is_numeric_marker(self, marker_info):
        """ตรวจสอบว่า marker เป็นตัวเลขหรือไม่"""
        try:
    ...
    def scan_for_markers_360(self):
        """หมุน gimbal 360 องศาเพื่อหา markers รอบทิศทาง"""
        print("Scanning 360° for red markers...")
        detected_markers = []
        
        # หมุน gimbal ไปรอบๆ เพื่อหา markers
        scan_angles = [-135, -90, -45, 0, 45, 90, 135, 180]
        
        for angle in scan_angles:
            print(f"Scanning at angle {angle}°...")
            self.ep_gimbal.moveto(pitch=0, yaw=angle, yaw_speed=120).wait_for_completed()
            time.sleep(0.3)  # รอให้กล้องเซ็ต
            
            # ตรวจสอบ markers หลายครั้งเพื่อความแม่นยำ
            for _ in range(3):
                markers = self.get_markers(max_age=0.2)
                for marker in markers:
                    if self.is_numeric_marker(marker["info"]):
                        # ถ้าเป็นตัวเลข ให้หมุนไปชี้ตรงที่เจอ marker
                        print(f"Found numeric marker: {marker['info']} at angle {angle}°")
                        
                        # คำนวณมุมที่ต้องหมุนไปชี้ marker
                        marker_x = marker["x"]
                        # ปรับมุมตาม position ของ marker ในภาพ
                        if marker_x < 320:  # ถ้า marker อยู่ทางซ้าย
                            target_angle = angle - 15
                        elif marker_x > 320:  # ถ้า marker อยู่ทางขวา
                            target_angle = angle + 15
                        else:
                            target_angle = angle
                        ...
                        # ยืนยันการตรวจจับอีกครั้ง
                        confirm_markers = self.get_markers(max_age=0.2)
                        for confirm_marker in confirm_markers:
                            if self.is_numeric_marker(confirm_marker["info"]) and confirm_marker["info"] == marker["info"]:
                                detected_markers.append({
                                    "id": str(confirm_marker["info"]),
                                    "angle": target_angle,
                                    "position": (confirm_marker["x"], confirm_marker["y"]),
                                    "size": (confirm_marker["w"], confirm_marker["h"])
                                })
                                print(f"Confirmed marker {confirm_marker['info']} at angle {target_angle}°")
                                break
                time.sleep(0.1)
        
        # รีเซ็ต gimbal กลับหน้า
        self.ep_gimbal.moveto(pitch=0, yaw=0, yaw_speed=120).wait_for_completed()
        
        # ลบ markers ที่ซ้ำกัน
        unique_markers = []
        for marker in detected_markers:
            is_duplicate = False
            for existing in unique_markers:
                if existing["id"] == marker["id"] and abs(existing["angle"] - marker["angle"]) < 30:
                    is_duplicate = True
                    break
            if not is_duplicate:
                unique_markers.append(marker)
        
        print(f"Total unique numeric markers detected: {len(unique_markers)}")
        for marker in unique_markers:
            print(f"  - Marker {marker['id']} at angle {marker['angle']}°")
...
def plot_maze(current_cell, visited, walls, path_stack, detected_markers=None, title="Real-time Maze Exploration"):
    ...
    # วาด markers ที่ตรวจพบ
    if detected_markers:
        for cell_pos, markers_list in detected_markers.items():
            for marker in markers_list:
                # วาดสัญลักษณ์ marker ที่ตำแหน่งกำแพง
                marker_x, marker_y = marker['wall_position']
                
                # วาดวงกลมสีแดงสำหรับ marker
                ax.plot(marker_x, marker_y, 'ro', markersize=15, markeredgecolor='darkred', markeredgewidth=2)
                
                # วาดตัวเลขภายในวงกลม
                ax.text(marker_x, marker_y, str(marker['id']), 
                       ha='center', va='center', fontsize=10, fontweight='bold', color='white')
                
                print(f"Plotted marker {marker['id']} at wall position ({marker_x:.1f}, {marker_y:.1f})")

    # วาดตำแหน่งหุ่นยนต์ปัจจุบัน
    cx, cy = current_cell
    ax.plot(cx, cy, 'go', markersize=12, label='Robot')
    
    # ตั้งค่าขอบเขตและรูปแบบกราฟ
...
class MazeSolver:
    ...
    def __init__(self, ctrl: Control):
        ...
        self.path_stack = [(0, 0)]
        self.walls = {}
        self.detected_markers = {}  # เก็บ markers ที่ตรวจพบ {cell_position: [marker_info]}
    ...
    def _calculate_wall_position(self, current_cell, angle):
        """คำนวณตำแหน่งของกำแพงบนแผนที่ตามมุมที่เจอ marker"""
        cx, cy = current_cell
        
        # แปลงมุมจาก gimbal เป็นทิศทางบนแผนที่
        # กำหนดให้ 0° = North, 90° = East, 180° = South, -90° = West
        
        if -45 <= angle <= 45:  # หน้า (North)
            return (cx, cy + 0.5)
        elif 45 < angle <= 135:  # ขวา (East)
            return (cx + 0.5, cy)
        elif angle > 135 or angle <= -135:  # หลัง (South)
            return (cx, cy - 0.5)
        else:  # ซ้าย (West) (-135 < angle <= -45)
            return (cx - 0.5, cy)

    def scan_for_markers(self, current_cell):
        """สแกนหา markers ในตำแหน่งปัจจุบัน"""
        print(f"\n=== Scanning for markers at {current_cell} ===")
        
        # สแกนหา markers รอบทิศทาง
        markers_found = self.ctrl.scan_for_markers_360()
        
        if markers_found:
            cell_markers = []
            for marker in markers_found:
                # คำนวณตำแหน่งของกำแพงที่เจอ marker
                wall_pos = self._calculate_wall_position(current_cell, marker['angle'])
                
                marker_info = {
                    'id': marker['id'],
                    'angle': marker['angle'],
                    'wall_position': wall_pos,
                    'detection_cell': current_cell
                }
                cell_markers.append(marker_info)
                
                print(f"Marker {marker['id']} detected at angle {marker['angle']}° -> Wall position {wall_pos}")
            
            # เก็บข้อมูล markers ที่ตรวจพบ
            if current_cell not in self.detected_markers:
                self.detected_markers[current_cell] = []
            self.detected_markers[current_cell].extend(cell_markers)
            
            print(f"Total markers found at {current_cell}: {len(cell_markers)}")
        else:
            print(f"No numeric markers detected at {current_cell}")
    ...
    def explore(self):
        ...
        while self.path_stack:
            ...
            # สแกนหา markers ในตำแหน่งปัจจุบัน
            self.scan_for_markers(current_cell)

            # อัพเดทแผนที่
            plot_maze(current_cell, self.visited, self.walls, self.path_stack, 
                     self.detected_markers, "Real-time Maze Exploration with Markers")
            
            print(f"\nPosition: {current_cell}, Orientation: {self.current_orientation} (Yaw: {self.ctrl.get_yaw_deg():.1f}°)")

            # แสดงข้อมูล markers ที่เจอ
            if current_cell in self.detected_markers:
                marker_ids = [m['id'] for m in self.detected_markers[current_cell]]
                print(f"[Markers at {current_cell}] IDs: {marker_ids}")

            # ทำการ map ก่อนเสมอ
            if current_cell not in self.maze_map:
                self._scan_and_map(current_cell, distances, is_outside)

            # ถ้าอยู่นอกแผนที่ ให้ backtrack กลับเข้ามา
            if is_outside:
        ...
        # สรุปผล markers ทั้งหมดที่ตรวจพบ
        print("\n=== MARKER DETECTION SUMMARY ===")
        total_markers = 0
        for cell_pos, markers_list in self.detected_markers.items():
            print(f"Cell {cell_pos}:")
            for marker in markers_list:
                print(f"  - Marker {marker['id']} at wall {marker['wall_position']}")
                total_markers += 1
        print(f"Total markers detected: {total_markers}")
        
        plot_maze(self.path_stack[-1], self.visited, self.walls, self.path_stack, 
                 self.detected_markers, "Final Map with All Detected Markers")
        finalize_show()
