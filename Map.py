...
class MazeMappingSystem:
    def __init__(self):
        ...
        self.colors = {
            ...
            self.FREE: '#95a5a6',       # Gray (same as UNKNOWN)
            self.WALL: '#95a5a6',       # Gray (same as UNKNOWN)
            self.VISITED: '#27ae60',    # Green
            self.CURRENT: '#27ae60',    # Green (same as VISITED)
            self.FRONTIER: '#95a5a6'    # Gray (same as UNKNOWN)
        }
        
        self.color_labels = {
            self.UNKNOWN: 'ไม่ทราบ',
            self.FREE: 'ว่าง',
            self.WALL: 'ผนัง',
            self.VISITED: 'เคยเดิน',
            self.CURRENT: 'ตำแหน่งปัจจุบัน',
            self.FRONTIER: 'จุดที่น่าสนใจ'
        }
    ...
    def load_map_from_csv(self, csv_file_path):
        """อ่านข้อมูลแผนที่จากไฟล์ CSV"""
        try:
            with open(csv_file_path, 'r', encoding='utf-8') as file:
                ...
                for row in csv_reader:
                    # แปลงข้อมูลในแถวเป็นตัวเลข
                    map_row = []
                    for cell in row:
                        try:
                            # รองรับค่า: 0=UNKNOWN, 1=FREE, -1=WALL
                            map_row.append(int(cell.strip()))
                        except ValueError:
                            # หากไม่สามารถแปลงเป็นตัวเลขได้ ให้ใช้ค่า UNKNOWN
                            map_row.append(self.UNKNOWN)
                    map_data.append(map_row)
                
                # ตรวจสอบขนาดแผนที่
                if len(map_data) != self.MAP_SIZE or any(len(row) != self.MAP_SIZE for row in map_data):
                    raise ValueError(f"CSV file must contain a {self.MAP_SIZE}x{self.MAP_SIZE} grid")
                
                # อัปเดตแผนที่
                self.map = np.array(map_data, dtype=int)
    ...
    def generate_random_maze(self):
        """สร้างเขาวงกตแบบสุ่ม"""
        # สร้างผนังแบบสุ่ม (ประมาณ 30% ของพื้นที่)
        for y in range(self.MAP_SIZE):
            for x in range(self.MAP_SIZE):
                if x == 3 and y == 3:  # จุดเริ่มต้นต้องว่างเสมอ
                    self.map[y][x] = self.FREE
                else:
                    self.map[y][x] = self.WALL if random.random() < 0.3 else self.FREE
    ...
    def ensure_connectivity(self):
        """ตรวจสอบให้แน่ใจว่าหุ่นสามารถเคลื่อนที่ได้จากจุดเริ่มต้น"""
        ...
        for dx, dy in directions:
            ...
            if self.is_valid_position(new_x, new_y):
                self.map[new_y][new_x] = self.FREE  # ให้แน่ใจว่ามีเส้นทางอย่างน้อยหนึ่งทาง
